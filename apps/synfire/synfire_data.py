...
class SynfireDatasetConv(torch.utils.data.Dataset):
    ...
    def gen_sample(self, batch_size, num_timesteps, ext_input_type):
        ...
        for i in range(batch_size):
            ...
            #<< still need random shift for conv unless I remove bias in batchnorm
            
            #add random perturbation to the amplitude
            input_mean[i,:] = patch.roll( rand_shift )*0.15*(1 + 0.1*torch.randn(self.input_dim, generator = self.gen) )+0.03*torch.randn(self.input_dim, generator = self.gen)
            input_std[i,:] = patch.roll( rand_shift)*0.2*(1 + 0.1*torch.randn(self.input_dim, generator = self.gen) )+0.03*torch.randn(self.input_dim, generator = self.gen)
